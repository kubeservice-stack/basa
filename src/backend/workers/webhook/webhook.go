...
import (
	...
	"github.com/kubeservice-stack/basa/src/backend/bus"
	"github.com/kubeservice-stack/basa/src/backend/bus/message"
	"github.com/kubeservice-stack/basa/src/backend/models"
	"github.com/kubeservice-stack/basa/src/backend/models/response"
	"github.com/kubeservice-stack/basa/src/backend/util/logs"
	"github.com/kubeservice-stack/basa/src/backend/workers"
)
...
func (w *WebhookWorker) exec(hook models.WebHook, data *message.HookMessageData) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("Basa-WebHook/%s", ua))
	if len(hook.Secret) > 0 {
		req.Header.Set("X-Basa-Signature", fmt.Sprintf("sha1=%x", sha1.New().Sum([]byte(hook.Secret))))
	}
